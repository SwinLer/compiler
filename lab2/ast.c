...
symbolTable myTable = {{0}, 0}; //符号表
symbol_scope_TX myScope = {{0}, 0}; //当前作用域的符号在符号表的起始位置序号，栈结构，顺序表

int i, j, t, counter = 0; //参数计数
int rtn, flag1, flag2, num;
int mem, stru_dec = 0, exp_ele = 0; //定义结构体，参数或结构成员变量
int rtn2;
char struct_name[33];
int switch_flag = 0, loop_flag = 0; //限制break位置
int left_required = 0;
int array_size = 0;
...
int semantic_Analysis(struct ASTNode *T, int type, int level, char flag, int command){
    ...
    if(T) {
        ...
        case EXT_VAR_DEF:   //处理外部说明,将第一个孩子(TYPE结点)中的类型送到第二个孩子的类型域
            ...
            semantic_Analysis(T->ptr[1], type, level, flag, command);
            break;
        case EXT_STRUCT_DEF:  //结构体外部声明
            semantic_Analysis(T->ptr[0], type, level, flag, command);
            break;
        case STRUCT_DEF: //结构体定义
        ...
        case FUNC_DEF://外部函数声明
            ...
            myTable.index = num+1;
            if(flag1 && !flag2){ //flag1：是否定义返回值void flag2：函数是否有return语句 
